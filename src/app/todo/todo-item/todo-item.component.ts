...
export class TodoItemComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.checkField.valueChanges.subscribe( value => { 
      ...
      let accion = new ToggleTodoAction( this.todo.id );
  ...
  terminarEdicion(){
    ...
    let accion = new EditarTodoAction(this.txtInput.value, this.todo.id);
    this.store.dispatch(accion);
  }

  borrarTodo(){
    let accion = new BorrarTodoAction(this.todo.id);
    this.store.dispatch(accion);
  }
